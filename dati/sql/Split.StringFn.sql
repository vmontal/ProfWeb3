...
CREATE FUNCTION pf_pianoLezioniUpdateFn (@idProfessore int, @String VARCHAR(1000), @tipo varchar(1)) RETURNS INT
...
BEGIN
	...
	WHILE @N <= @TOT
		BEGIN
			...
			--IF EXISTS (SELECT idLezione FROM pf_pianoLezioni WHERE idProfessore = @idProfessore AND idMateria = @idMateria)
			--BEGIN
